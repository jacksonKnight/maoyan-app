<template>
  <div class="detail">
    <div class="detail-top">
      <span class="iconfont icon-zuojiantou" @click="height"></span>
      <span>猫眼电影</span>
    </div>
    <div class="detail-nav">
      <div class="detail-nav-left">
        <div class="detail-nav-img">
          <img :src="detailes.images" class="auto-img" alt />
          <!-- {{movies}} -->
        </div>
        <div class="detail-rig">
          <div @click="aaa">{{detailes.title}}</div>
          <div>{{detailes.original_title}}</div>
          <div>
            <span>{{detailes.rating.average}}&nbsp;&nbsp;</span>
            <span>({{detailes.rating.stars}}万人评)</span>
          </div>
          <div>{{getGenres(detailes.genres)}}</div>
          <div>{{detailes.durations[1]}}</div>
    ...
    <div class="detail-fix">
      <div class="detail-ul-container">
        <div class="detail-ul" >
          <li @click="cur = index" :class="{active : cur === index}" v-for="(item,index) in todo" :key="index">{{item}}{{getItem(dates)}}</li>
        </div>
      </div>
    ...
    <div class="detail-end" >
      <div class="detail-end-ul">
        <div  class="detail-end-list" v-for="(m,index) in cinema" :key="index">
          ...
          <div class="address">
            <div class="first"><span>{{m.address}}</span></div>
            <div class="second"><span>{{m.distance}}</span></div>
          </div>
          <div class="decount">
